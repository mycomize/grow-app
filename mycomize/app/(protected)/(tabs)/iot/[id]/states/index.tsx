            ...
            <HStack className="items-center justify-between">
              ...
              <Badge variant="solid" action="success">
                <Text size="xs" className="text-white">
                  {filteredStates.length} entities
                </Text>
              </Badge>
            </HStack>
          ...
          <Card key={domain} className="bg-background-0">
            <VStack className="p-4" space="sm">
              ...
              {domainStates.map((state) => (
                <Card key={state.entity_id} className="bg-gray-50 p-3">
                  <HStack className="items-center justify-between">
                    ...
                    <Switch
                      value={enabledStates.has(state.entity_id)}
                      onValueChange={(value) => {
                        setEnabledStates((prev) => {
                          const newSet = new Set(prev);
                          if (value) {
                            newSet.add(state.entity_id);
                          } else {
                            newSet.delete(state.entity_id);
                          }
                          return newSet;
                        });
                      }}
